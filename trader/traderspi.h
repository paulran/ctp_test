...
#include "countstatis.h"
#include "datasaved.h"

class CTraderSpi : public CThostFtdcTraderSpi
{
public:
    CTraderSpi(CThostFtdcTraderApi *api, const string &frontAddress, const string &brokerID, const string &userID,
...
public:
    ...
    int ReqOrderAction(TThostFtdcExchangeIDType exchangeID, TThostFtdcInstrumentIDType instrumentID,
                       const char *orderRef, TThostFtdcOrderActionRefType &orderActionRef);

private:
    CThostFtdcTraderApi *api_ = nullptr;
    string frontAddress_;
    string brokerID_;
    string userID_;
    string userProductInfo_;
    string authCode_;
    string appID_;
    string password_;
    string investorID_;

    CountStatis orderInsertCountStatis_;
    CountStatis orderCancelCountStatis_;

    int requestId_ = 0;
    int requestReturn_ = 0;
    int orderRefId_ = 0;                              // 报单引用，本地会话全局唯一编号，必须保持递增；可由用户维护，也可由系统自动填写。一定为数字。需要转换为 TThostFtdcOrderRefType 类型使用。
    TThostFtdcOrderActionRefType orderActionRef_ = 0; // 报单操作（撤单）引用
